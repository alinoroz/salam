...
define('BOT_TOKEN', '240125350:AAHpHKQMXrwh7CtMqu4seEgZ3Elup5YwWDs');
...
Copy Right 2016©","parse_mode"=>"html"));
    ...
    }else if ($matches[0] == "/sendtoall"&& $chat_id == 199498852) {
		...
		for($y=0;$y<count($banid);$y++)
      {
        apiRequest("sendMessage", array('chat_id' => $banid[$y], "text" => $sendtotext));
          ...
          apiRequest("sendMessage", array('chat_id' => 199498852, "text" => "Sent To ".count($banid)." Members"));
...
🗣پیام ارسال شد");
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[1].'/setwebhook?url=https://s3pvresan-vpnnariman.rhcloud.com/'.$chat_id.'/pvresan.php');
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "🚀 ربات شما با مـوفقیت آپدیت شد ♻️"));


    }
    }
    else if ($matches[0] != "/update"&& $matches[1]==""&&$chat_id != 199498852) {
      if (strpos($text, ":")) {
...
🚀 @nawr_i_man_bot"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == 199498852) {
